...
log4js.configure({
    appenders: {
        console: { type: 'console' }, //记录器1:输出到控制台 
        data_file: {//记录器2：输出到日期文件
            type: "dateFile",
            filename: __dirname + '/timeLogFile',
        ...
        log_file:{//记录器3：输出到文件
            type : 'file',
            filename: __dirname + '/logFile',//文件目录，当目录文件或文件夹不存在时，会自动创建
        ...
        error_file:{//：记录器4：输出到error log
            type: "dateFile",
            filename: __dirname + '/logFile_error',//您要写入日志文件的路径
            alwaysIncludePattern: true,//（默认为false） - 将模式包含在当前日志文件的名称以及备份中
            daysToKeep:30,//时间文件 保存多少天，距离当前天daysToKeep以前的log将被删除
    ...
    categories: {
        ...
        // logFile: { appenders: ['console', 'app'], level: 'ALL' },
