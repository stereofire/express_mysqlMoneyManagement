...
router.post('/', upload, function (req, res, next) {
  if (req.session.islogin) {
    ...
    if (req.query.fileUpload == "true") {
      logger.info("进入TcoursePlansAdmin?fileUpload=ture");
                  ...
                  connection.query($sql.TcoursePlansUpload_queryLastProductID, function (err, result) {
                    if (err) {
                      ...
                      for (var j = 1; j < coursePlansInfo.length; j++) {
                        ...
                        logger.info("j - 1:",j - 1,lastProductID, num, textBookID[j - 1]);
                      ...
                      logger.info("TcoursePlansUploadParams:", TcoursePlansUploadParams);
    ...
    if (req.query.addCoursePlan == "true") {
      logger.info('进入TcorpInfoAdmin?addCoursePlan=true，get FormData Params: ', req.body);
      /*插入新增教材计划信息*/
      userDao.addCoursePlanInfo(res, req);
    }
    if (req.query.querySiftCoursePlans == "true") {
      logger.info('进入TcorpInfoAdmin?querySiftCoursePlans=true，get FormData Params: ', req.body);
      /*筛选教材计划信息*/
